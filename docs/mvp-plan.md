...
The organizing vision and staged operational plan for flowstore. For the data-model contract see [SCHEMA.md](../SCHEMA.md); for the on-disk layout see [FILE-MODEL.md](../FILE-MODEL.md); for architectural rationale see [AGENTS.md](../AGENTS.md); for the translation/fidelity experiment that gates runner-based testing see [TRANSLATION-POC.md](./translation-poc.md); for the systems view of the client-materials → spec → tests → targets loop and what would be required to make it self-optimizing see [docs/optimization-loop.md](./optimization-loop.md); for the canonical testing framework — units of testing, what's built, and the plan for the rest — see [docs/testing-plan.md](./testing-plan.md); for non-technical-user UX of GitHub primitives — dirty state, conflicts, OAuth — see [docs/github-ux.md](./github-ux.md).
...
- **1g** `models/` schema + loader. `flowstore://spec/models/v0` defines `{ models, default, roles }` with free-form string keys (no `providers` map, no `kind` enum — schemas validate shape, not value membership). Loader merges `models/*.json` files. Built-in data carries the four currently supported Google models (Gemini 2.5 / 3.x preview). No runtime consumer yet — Simulate/chat panels stay BYOK Google in Phase 1; provider-adapter wiring lands in Phase 2.
- **1h** Per-file AJV validation in the loader for `flowstore.json`, `models/*.json`, `knowledge/glossary.json`, `knowledge/tables/<id>.meta.json` — errors flow through `LoadError[]`. New `KnowledgeTableMetaSchema` (flowstore://spec/knowledge-table/v0). Existing canvas inline display (per-flow red border + tooltip, per-edge red stroke) continues to surface dangling references unchanged.
...
- **Comments anchored to flows.** Per-uuid additive files at `comments/<uuid>.comment.json` (`flowstore://meta/comment/v0`). Flat threading (no replies), resolve flips `resolved: true` rather than deleting (Git-shaped audit trail kept). `CommentsSection` lives at the bottom of `FlowInspector`; small amber unresolved-count badge on each flow node on the canvas. Writes use GitHub's Contents API (`PUT /contents/{path}`) so concurrent posts serialize cleanly — `mode: "create"` for new comments skips the existence probe, `mode: "update"` for resolve/reopen fetches the current blob sha. v1 anchor enum is `flow` only; broaden when designers ask. Author is hardcoded to `"user"` until a `gh api /user` echo lands on PAT save. **Persistence asymmetry, intentional:** comment writes commit to GitHub immediately, while spec edits stay in-memory until the user clicks Save. Comments are conflict-free per-uuid additions (safe to auto-commit) and don't have a "discard" mental model; spec edits touch many files at once and benefit from a curated commit boundary. Consequence: a comment posted while the spec is dirty references a flow shape that isn't on `main` yet — acceptable for v0, revisit if pilot designers hit confusion. **No delete, only resolve.** Resolved comments stay on disk as `resolved: true` files for the audit trail; there is no UI path to delete. Over a long-lived repo this accumulates everywhere — `comments/` bloats on disk, every `FlowInspector` open renders the full resolved history in `CommentsSection`, and project loads slow as the comment count grows. Mitigation when it matters: (a) a delete affordance alongside resolve — resolve means "addressed, keep visible for context," delete means "noise / mistake, shouldn't pollute current state"; git history preserves the audit trail either way, so the design-decision rationale ("Git-shaped audit trail kept") survives a real delete. (b) An archive sweep for resolved-and-older-than-N-days into `comments/archive/` if hand-curated delete proves insufficient at scale.

Still ahead in Phase 2 (see [docs/testing-plan.md](./testing-plan.md) for the canonical framework, current state, and sequenced plan):
...
- **`flowstore.json` is minimal** — just `{ "$schema": "flowstore://spec/project/v0" }`. Agents implicit from filesystem (presence of `agents/` directory); compile targets are CLI flags; project name derived from directory; client lives in `agent.meta.client`.
...
- **Non-technical-user UX for GitHub primitives** — dirty-state indicator, conflict modal rework, branch → draft language, OAuth in place of PAT. Near-term plan (dirty pill + Cmd+S + IndexedDB draft recovery) plus tracked deferred work in [docs/github-ux.md](./github-ux.md). Pull deferred items in when a pilot designer hits the friction.
