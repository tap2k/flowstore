...
export function ScenariosPanel() {
  ...
  // Inline "Generate" form. null = closed; object = author is typing the
  // name + notes that ground the LLM generation.
  ...
  async function runGenerate() {
    ...
    try {
      ...
      const ctx = { name: name || undefined, notes: notes || undefined };
      const declared = collectDeclaredVariables(spec);
      const caps = collectMockableCapabilities(spec);
      const vars =
        declared.length > 0
          ? await generateContextVars(spec, apiKey, geminiModel, declared, ctx)
          : {};
      const mocks =
        caps.length > 0
          ? await generateCapabilityMocks(spec, apiKey, geminiModel, caps, vars, ctx)
          : {};
      // Translate mock returns (cap_name at runtime → cap_id for the
      // scenario shape).
      const nameToId = new Map<string, string>();
      for (const cap of spec.agent.capabilities ?? []) nameToId.set(cap.name, cap.id);
      const scenarioMocks: Record<string, ScenarioMockBehavior> = {};
      for (const [capName, returns] of Object.entries(mocks)) {
        const cid = nameToId.get(capName);
        if (!cid) continue;
        scenarioMocks[cid] = { kind: "static", returns };
      }
      ...
      saveScenario({
        ...
        ...(Object.keys(scenarioMocks).length > 0 ? { mocks: scenarioMocks } : {}),
      });
      setSelectedId(id);
      setGenerating(null);
    } catch (e) {
        ...
        <div className="space-y-2 border-b border-zinc-200 bg-white px-3 py-2 text-[11px]">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="What's the world the agent operates in? e.g. Sophie has an active policy; claim filing succeeds; adjuster scheduled."
              rows={3}
