...
Marks a variable the deployment **may hand to the session at start** — a dialer record for an outbound agent, a screen-pop CRM fetch, caller ID. A permission, not a guarantee: an inbound or anonymous session simply arrives without the value. Default false (absent).

`provided` is the only gate through which test-fixture vars reach the agent's initial state. A persona/case `vars` dict is a *character sheet*: at run time the editor ships `fixture vars ∩ provided` as `context_vars` (runner bag) / bakes the same subset into the compiled prompt (prompt mode). Everything else is edit-time ground truth — persona prompt generation, mock consistency, assertions — that the agent must **earn** through the conversation or a capability return. This is what keeps tests honest: an unmarked `policy_id` in a persona cannot leak into the agent and trivially satisfy the elicitation flow under test.
...
- `vars?: Record<string, unknown>` — **character-intrinsic** vars only: facts true of this character in *every* test (their name, the policy id their identity is keyed on). Free-form `{variable_name: value}` dict, coerced against `agent.variables`. A character sheet, not a seed: only `provided`-declared keys ship to the agent at session start; the rest ground persona-prompt generation, mock consistency, and assertions, and reach the agent only when the persona says them or a mock returns them. Situational vars go on the case. **Populated at creation time** by the editor's generator when the spec declares variables; hand-authored personas should fill all declared variable slots.
- `mocks?: Record<capability_id, MockBehavior>` — character-intrinsic per-capability returns, keyed by capability id (e.g. the `verify_policy` return keyed on this caller's identity). Caps not listed fall through to the live capability. **Populated at creation time** by the editor's generator for every mockable capability (those whose output variables are declared in the spec); hand-authored personas should mock all capabilities whose outputs the agent reads.
