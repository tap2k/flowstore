...
Model selection — including any per-agent override — is **execution**, not spec. The deployment-side runtime reads a sibling `execution.json` (or equivalent) to decide which model dispatches each agent. Spec stays model-agnostic; execution config maps abstract agent identity to provider+wire-id+key. Wire ids, API keys, endpoint URLs, region-specific routing, telephony / STT / TTS / voice / VAD config all belong there too.
