...
**Shared concern across graph-native targets: chatty boundaries.** At a flow boundary, the source flow speaks a transition reply, then control hands to the destination. The architectural question is *whether the destination's prompt also gets an LLM call on the same turn* — needed when the destination is **terminator-shaped** (a closer / confirmation flow whose script *is* the meaningful content of the transition turn, e.g. Tala's `commit_to_pay_close`, `partial_commitment_close`). The monolithic system-prompt path sidesteps the question because one LLM call sees every flow and authors both the source's hand-off and the destination's opener in one go. Graph-native targets each handle it differently:

- **Pipecat solves it natively.** `NodeConfig.respond_immediately` (default `True`) queues an `LLMRunFrame()` against the destination node's `task_messages` the moment a transition lands (see [`pipecat_flows.manager._set_node`](https://reference-flows.pipecat.ai/en/stable/_modules/pipecat_flows/manager.html)). The translator's job is to *not override the default* — emit nodes with `respond_immediately=True` (or omit, since it defaults true) and Pipecat's runtime speaks the destination's opener for free. `context_strategy: APPEND` (default) preserves the source's transition reply in the context the destination sees, so the opener has continuity. Voice latency at chatty transitions is mitigated by `pre_actions` (TTS a holding line before the destination LLM runs) — first-class in the framework.
- **LangGraph translator needs the follow-up.** `Command(goto=...)` switches state but doesn't auto-invoke the destination node's LLM call on the same turn. The translator should fire the destination's `_llm_call` (already `@task`-wrapped per [TRANSLATION-POC.md § F3](./TRANSLATION-POC.md)) when the source's transition reply is non-empty and the destination is terminator-shaped. The detection heuristic is structural (destination has scripts AND no LLM-method exit conditions).
- **uxflows-runner needs the same fix.** Same heuristic, wired into `text_session._run_one_turn`'s follow-up logic. Full diagnosis and trade-off discussion in [`uxflows-runner/RUNNER-PLAN.md` § Chatty-boundary follow-up](../uxflows-runner/RUNNER-PLAN.md#chatty-boundary-follow-up-open).

The chosen default across all three: **always emit the destination opener on chatty transitions; voice surfaces may speculatively pipeline (e.g. Pipecat's `pre_actions`) to mask the extra TTFT.** Correctness as default, voice-latency optimization as a per-target concern.
...
Chatty-boundary behavior (see [intro](#export-targets)) requires translator-side follow-up: `Command(goto=...)` switches state without firing the destination node's LLM call on the same turn. Validated on FNOL because FNOL's flow boundaries are non-chatty; Tala-shaped specs would expose it. The fix wraps cleanly with the existing `@task`-decorated `_llm_call` wrapper (see [TRANSLATION-POC.md § F3](./TRANSLATION-POC.md)) and uses the same structural detection heuristic as the runner.
