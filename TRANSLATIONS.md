...
- **LangGraph translator: needs follow-up + source-side suppression.** `Command(goto=...)` switches state without invoking the destination's LLM call. The translator should (a) detect terminator-shaped destinations via the structural heuristic below, (b) render the source flow's prompt with a "speak nothing on this transition" directive for the relevant exit, and (c) fire the destination's `@task`-wrapped `_llm_call` ([TRANSLATION-POC.md § F3](./TRANSLATION-POC.md)) as a follow-up.
- **uxflows-runner: needs the same fix.** Same detection, same source-side suppression in the route-tag protocol, wired into `text_session._run_one_turn`'s follow-up logic. Full diagnosis and trade-off discussion in [`uxflows-runner/RUNNER-PLAN.md` § Chatty-boundary follow-up](../uxflows-runner/RUNNER-PLAN.md#chatty-boundary-follow-up-open).

**Detection heuristic (shared across LangGraph + runner).** A transition is "chatty" when the destination flow has scripts AND no LLM-method exit conditions (i.e., authored to speak a script then deterministically end or unconditional-route). Pipecat doesn't need this heuristic — it always does C — but for translators that have to opt in, the heuristic gates the source-side suppression and the destination follow-up to the subset of transitions that need them. Non-chatty transitions stay one-call, source speaks normally.
...
Chatty-boundary behavior (see [intro](#export-targets)) requires translator-side follow-up plus source-side suppression: detect terminator-shaped destinations via the structural heuristic, suppress the source flow's spoken reply on the relevant exit, and fire the destination's `@task`-wrapped `_llm_call` (see [TRANSLATION-POC.md § F3](./TRANSLATION-POC.md)). Same heuristic and same target behavior as the runner. Validated on FNOL because FNOL's flow boundaries are non-chatty; Tala-shaped specs would expose it.
